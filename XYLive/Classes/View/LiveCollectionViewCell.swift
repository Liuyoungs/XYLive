...
class LiveCollectionViewCell: UICollectionViewCell {
    ...
    override func layoutSubviews() {
        ...
        self.bottomView.frame = CGRect.init(x: 0, y: self.contentView.Height-80, width: self.contentView.Width, height: 44)
        self.headerView.frame = CGRect.init(x: 0, y: 0, width: self.Width, height: 110)
        playWithFLV(flv: _flv)
    }
    
    
    //开始直播
    fileprivate func playWithFLV(flv:String) {
...
//        }
    ...
    fileprivate lazy var emitterLayer: CAEmitterLayer = {
        ...
        //发射器在xy平面的中心位置
        emitter.emitterPosition = CGPoint.init(x: self.moviePlayer.view.Width - 50, y: self.moviePlayer.view.Height - 50)
        //发射器尺寸
    ...
    lazy var bottomView: LiveBottomView = {
        let bottomView = LiveBottomView.instanceFromNib()
    ...
    /** 头部视图*/
    lazy var headerView: LiveHeaderView = {
        let headerView = LiveHeaderView.instanceFromNib()
